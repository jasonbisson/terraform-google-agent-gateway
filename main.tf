...
locals {
  ...
  agent_gateway_dns_peering_config_effective = (
    ...
    : {
      ...
      target_project = coalesce(try(var.agent_gateway_dns_peering_config.target_project, null), module.foundation.project_id)
...
module "foundation" {
  ...
  providers = {
    ...
    google-beta = google-beta.project_creation
  }
...
resource "google_storage_bucket" "cloudbuild" {
  project                     = module.foundation.project_id
  name                        = coalesce(var.cloudbuild_bucket_name, "${module.foundation.project_id}_cloudbuild")
  location                    = var.region
...
module "mcp_services" {
  ...
  bind_invoker_sa  = var.enable_agent_engine
